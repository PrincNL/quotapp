...
import { Scale, RotateCcw, Info, Heart } from "lucide-react";
...
export function BMICalculatorClient() {
  ...
  const bereken = useCallback(() => {
    const lengteM = parseFloat(lengte.replace(",", ".")) / 100;
  ...
  const resultText = resultaat
    ? `BMI: ${resultaat.bmi} - ${resultaat.range.label}`
    : "";
        ...
        <div>
          ...
          >
            <div className="flex items-center justify-center gap-3 mb-4">
              ...
              <FavoriteButton toolName="BMI" variant="icon" />
            </div>
            ...
            <AnimatePresence mode="wait">
              {resultaat ? (
                ...
                >
                  {/* BMI Display */}
                  <motion.div
                    className="text-center p-6 rounded-xl result-highlight"
          ...
          <div className="mt-8 flex justify-center">
            <FeedbackForm toolName="BMI Calculator" />
          </div>
        </div>

        <StickyAd slot="sidebar-ad-bmi" />
      </div>
    </div>
  );
}
