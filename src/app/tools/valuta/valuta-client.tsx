...
export function ValutaConverterClient() {
  ...
  const resultText = resultaat
    ? `${bedrag} ${van} = ${formatNumber(resultaat.value)} ${naar}`
    : "";
        ...
        <div>
          ...
          >
            <div className="flex items-center justify-center gap-3 mb-4">
              <div className="p-3 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-600 text-white shadow-lg">
                <TrendingUp className="w-8 h-8" />
              </div>
              <FavoriteButton toolName="Valuta" variant="icon" />
            </div>
          ...
          <div className="mt-8 flex justify-center">
            <FeedbackForm toolName="Valuta Converter" />
          </div>
        </div>

        <StickyAd slot="sidebar-ad-valuta" />
      </div>
    </div>
  );
}
