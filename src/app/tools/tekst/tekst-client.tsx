...
export function TekstToolsClient() {
  ...
  // Record usage when text is analyzed
  useEffect(() => {
    if (tekst.length > 50) {
      recordToolUsage("Tekst");
    }
  }, [tekst.length > 50]); // Only record when significant text is entered
  ...
  const calculateStats = useCallback(() => {
    ...
    const leestijd = Math.ceil(woorden / 200);
        ...
        <div>
          ...
          >
            <div className="flex items-center justify-center gap-3 mb-4">
              ...
              <FavoriteButton toolName="Tekst" variant="icon" />
            </div>
          ...
          <div className="mt-8 flex justify-center">
            <FeedbackForm toolName="Tekst Tools" />
          </div>
        </div>

        <StickyAd slot="sidebar-ad-tekst" />
      </div>
    </div>
  );
}
